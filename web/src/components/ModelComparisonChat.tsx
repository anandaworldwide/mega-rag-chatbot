...
const ModelComparisonChat: React.FC<ModelComparisonChatProps> = ({ siteConfig, savedState, onStateChange }) => {
  ...
  const handleTemperatureChange = useCallback((modelKey: "A" | "B", value: number) => {
    if (modelKey === "A") {
  ...
  const handleRandomize = useCallback(() => {
    const getRandomModel = () => modelOptions[Math.floor(Math.random() * modelOptions.length)].value;
  ...
  const handleSubmit = async (e: React.FormEvent, query: string) => {
    ...
    console.log("Current message count before adding new message - A:", messagesA.length, "B:", messagesB.length);
    ...
    try {
      ...
      console.log("Chat history A length:", historyA.length, "B length:", historyB.length);
  ...
  const handleExport = async () => {
    try {
      ...
      const filename = contentDisposition?.split("filename=")[1]?.replace(/"/g, "") || "model-comparison-votes.csv";
  ...
  const chatInputProps = {
    ...
    handleEnter: (e: React.KeyboardEvent<HTMLTextAreaElement>, query: string) => {
  ...
  return (
    <div className="flex flex-col h-full">
            ...
            <div key={modelKey} className="flex flex-col min-w-0">
              ...
              <div className="flex flex-col gap-2 mb-4">
                <select
                  ...
                  onChange={(e) => handleModelChange(modelKey as "A" | "B", e.target.value)}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => handleTemperatureChange(modelKey as "A" | "B", parseFloat(e.target.value))}
                    className="flex-grow"
                    disabled={conversationStarted}
                  />
                  <span className="text-sm">{modelKey === "A" ? temperatureA : temperatureB}</span>
                </div>
              </div>
              <div className="flex-grow overflow-y-auto border rounded-lg">
                {(modelKey === "A" ? messagesA : messagesB).map((message: ExtendedAIMessage, index: number) => (
                  <MessageItem
                    key={index}
                    message={message}
                    index={index}
                    isLastMessage={index === (modelKey === "A" ? messagesA : messagesB).length - 1}
                    loading={loading}
      ...
      {conversationStarted && !loading && messagesA.length > 0 && messagesB.length > 0 && !hasVoted && (
        <div className="p-6 border rounded-lg bg-gray-50">
          <div className="text-center mb-4">
            <h3 className="text-xl font-medium">Which response was more helpful?</h3>
            <p className="text-sm text-gray-600 mt-2">
              If neither response was helpful, skip and try a different question
            </p>
          </div>
      ...
      {modelError && <div className="text-red-500 text-sm mb-4">{modelError}</div>}
      ...
      {isAdmin && (
        <div className="mt-8 flex justify-center gap-4">
          ...
          <Link href="/admin/model-stats" className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
            View Results
          </Link>
        </div>
      )}
        ...
        >
          <div className="bg-white p-6 rounded-lg shadow-xl text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-4 justify-center">
              ...
              <Link
                href="/"
                onClick={() => logEvent(`${PAGE}_post_vote`, PAGE, "Return Home")}
...
const VoteModal: React.FC<VoteModalProps> = ({ isOpen, onClose, onSubmit, selectedModel }) => {
  ...
  const handleSubmit = () => {
    ...
    if (!hasCheckedReason && !comments.trim()) {
      setValidationError("Please select at least one reason or provide a comment");
      return;
    }
    setValidationError(null);
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <div className="mx-auto max-w-sm rounded bg-white p-6">
          <h2 className="text-lg font-medium mb-4">
            {selectedModel ? `What made Response ${selectedModel} better?` : "Compare Responses"}
          </h2>
