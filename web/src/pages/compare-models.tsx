...
export const getServerSideProps: GetServerSideProps = async ({ req }) => {
  ...
  const isAdminAllowed = await isAdminPageAllowed(req as NextApiRequest, undefined, siteConfig);

  if (!siteConfig?.enableModelComparison && !isAdminAllowed) {
    return {
...
const InfoModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-lg p-6 max-w-2xl shadow-xl" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-2xl font-bold mb-4">Understanding AI Models</h2>
        <div className="text-gray-600 space-y-4">
          <p>Help us compare different AI models by choosing which response is more helpful.</p>
          <p>
            If neither response is helpful, you can skip and try a different question - this helps avoid biasing our
            data when neither response meets expectations.
          </p>
          <p>
            When we adjust the &quot;temperature&quot; setting, we&apos;re telling the AI how creative to be. A low
            temperature (like 0) means the AI will be very focused and consistent - great for factual answers. A higher
            temperature (like 0.7) allows for more creativity and variety - better for brainstorming and casual
            conversation.
          </p>
          <p>
            Your feedback helps us understand which AI &quot;chef&quot; and which &quot;cooking style&quot; works best
            for different situations. This helps us provide better service to everyone!
          </p>
        </div>
...
const ModelComparison: React.FC<ModelComparisonProps> = ({ siteConfig, isSudoAdmin, isHidden }) => {
  const [savedState, setSavedState] = useLocalStorage<SavedState>("modelComparisonState", {
    modelA: "gpt-4",
