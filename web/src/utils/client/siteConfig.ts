...
export const getCollectionsConfig = (siteConfig: SiteConfig | null) => siteConfig?.collectionConfig ?? {};
...
export const getSiteName = (siteConfig: SiteConfig | null) => siteConfig?.name ?? "The AI Chatbot";

// Get site shortname (defaults to 'AI Chatbot')
export const getShortname = (siteConfig: SiteConfig | null) => siteConfig?.shortname ?? "AI Chatbot";

// Get site tagline (defaults to 'Explore, Discover, Learn')
export const getTagline = (siteConfig: SiteConfig | null) => siteConfig?.tagline ?? "Explore, Discover, Learn";

// Get parent site URL (defaults to an empty string)
export const getParentSiteUrl = (siteConfig: SiteConfig | null) => siteConfig?.parent_site_url ?? "";

// Get parent site name (defaults to an empty string)
export const getParentSiteName = (siteConfig: SiteConfig | null) => siteConfig?.parent_site_name ?? "";
...
export const getLibraryMappings = (siteConfig: SiteConfig | null) => siteConfig?.libraryMappings ?? {};
...
export const getEnableSuggestedQueries = (siteConfig: SiteConfig | null) => siteConfig?.enableSuggestedQueries ?? false;
...
export const getLoginImage = (siteConfig: SiteConfig | null) => siteConfig?.loginImage ?? null;
...
export const getHeaderConfig = (siteConfig: SiteConfig | null): HeaderConfig => {
  return siteConfig?.header ?? { logo: "", navItems: [] };
};

// Get footer configuration (defaults to empty links array)
export const getFooterConfig = (siteConfig: SiteConfig | null): FooterConfig => {
  return siteConfig?.footer ?? { links: [] };
};

// Check if login is required (defaults to true)
export const getRequireLogin = (siteConfig: SiteConfig | null): boolean => siteConfig?.requireLogin ?? true;

// Check if temporary sessions are allowed (defaults to false)
export const getAllowTemporarySessions = (siteConfig: SiteConfig | null): boolean =>
  siteConfig?.allowTemporarySessions ?? false;

// Check if the "All Answers" page is allowed (defaults to false)
export const getAllowAllAnswersPage = (siteConfig: SiteConfig | null): boolean =>
  siteConfig?.allowAllAnswersPage ?? false;

// Get enabled media types (defaults to all types: text, audio, and youtube)
export const getEnabledMediaTypes = (siteConfig: SiteConfig | null): ("text" | "audio" | "youtube")[] =>
  siteConfig?.enabledMediaTypes ?? ["text", "audio", "youtube"];

// Get whether related questions should be shown (defaults to true)
export const getShowRelatedQuestions = (siteConfig: SiteConfig | null): boolean =>
  siteConfig?.showRelatedQuestions ?? true;
